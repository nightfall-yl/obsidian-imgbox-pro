...
- 修复复制图片后无法直接粘贴进 `.md` 的问题。此前为满足 `editor-drop-paste` ESLint 规则而在 `editor-paste` 处理器中对含文件粘贴调用 `evt.preventDefault()`，抑制了 Obsidian 默认的附件插入，而插件自身并不补插，导致图片粘贴失效。现移除该拦截，恢复 Obsidian 默认粘贴（附件仍由插件流水线本地化），并在 `eslint.config.mjs` 中为该处理器豁免 `editor-drop-paste` 规则并注明原因。
